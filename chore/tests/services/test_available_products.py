...
from shop.factories.product import ProductFactory, PublishedProductFactory
...
class AvailableProductsServiceTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.rental_period = RentalPeriodFactory(start_datetime=datetime.datetime(2019, 11, 1, 0, 0, 0),
                                                end_datetime=datetime.datetime(2019, 11, 2, 0, 0, 0))
        cls.service = AvailableProductsService(datetime.datetime(2019, 10, 1, 0, 0, 0),
        ...
        cls.order_item = OrderItemFactory(order=cls.order, product=cls.product2)
        cls.order_item = OrderItemFactory(order=cls.order, product=cls.product3)
